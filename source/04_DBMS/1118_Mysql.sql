...
-- DDL(제약조건 FK, 시퀀스 없음)
-- DML(and:&&, or:||, 연결연산자대신 concat 사용, outer join)
...
-- 시퀀스 대체, FK제약조건 (major : mcode시퀀스, mname, moffice) / student(sno, sname, mcode:FK)
...
create table student(
    ...
    mcode int,
    primary key(sno),
    foreign key(mcode) references major(mcode) -- FK제약조건
);
insert into major (mname,moffice) values ('컴공','901호');
insert into major (mname,moffice) values ('인공지능','a601호');
...
select * from student;
-- 학번, 이름,  학과번호, 학과명, 사무실(학생없는 학과도 출력)
...
select pno, pname, pay from personal where pay between 2000 and 5000;
...
select pno, pname, dname
    from personal p left outer join division d 
    on p.dno=d.dno;
-- 7. 사번, 이름, 상사이름
...
select sysdate(); -- MySQL은 select절 가능
