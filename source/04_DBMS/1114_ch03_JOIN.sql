...
SELECT *                 -- (3)
    FROM EMP E, DEPT D   -- (1) 이루 2,3번 줄에서 테이블의 별칭으로만 사용
    WHERE ENAME='SCOTT' AND E.DEPTNO=D.DEPTNO;  -- (2)
SELECT EMPNO, ENAME, JOB, MGR, HIREDATE, SAL, COMM, D.DEPTNO, DNAME, LOC --D.*가능
    FROM EMP E, DEPT D
    WHERE E.DEPTNO=D.DEPTNO;
    -- 급여가 2000이상인 직원의 이름, 직책, 급여, 부서명, 근무지
    ...
    -- EX. JOB이 'SALESMAN'이거나 'MANAGER'인 사람의 이름, 급여, 상여, 연봉, 부서명
        -- 연봉이 큰 순으로 정렬. 단, 연봉 = (SAL+COMM)*12
        SELECT ENAME, SAL, COMM, (SAL+NVL(COMM,0))*12 ANNUALSAL, DNAME FROM EMP E, DEPT D
        WHERE E.DEPTNO=D.DEPTNO AND JOB IN ('SALESMAN','MANAGER')
        ORDER BY (SAL+NVL(COMM,0))*12; -- ORDER BY ANNUALSAL DESC
    -- EX. COMM이 NULL이거나 0인 사원 중 SAL이 1200이상인 사원의 이름, 급여, 부서번호, 부서명
        -- 부서명순, 급여 큰순 정렬
...
-- ★ 4. OUTER JOIN : EQUI/SELF-JOIN 시 조건에 만족하지 않는 행까지 나타나게
...
SELECT W.ENAME, W.SAL, W.JOB, NVL(M.ENAME,'없음') MANAGERNAME
    FROM EMP W, EMP M
    WHERE W.MGR=M.EMPNO(+);
--4. 이름, 급여, 부서명, 직속상사명
...
SELECT W.ENAME, W.SAL, D.DEPTNO, DNAME, LOC, NVL(M.ENAME,'없음') MANAGERNAME
    FROM EMP W, EMP M, DEPT D
    WHERE W.DEPTNO=D.DEPTNO AND W.MGR=M.EMPNO(+);
...
SELECT W.ENAME, M.ENAME, P.ENAME
    FROM EMP W, EMP M, EMP P
    WHERE W.MGR=M.EMPNO AND M.MGR=P.EMPNO;
