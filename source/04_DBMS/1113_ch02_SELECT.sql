...
DESC EMP;
    -- EMP테이블의 구조
SELECT EMPNO, ENAME, SAL, JOB FROM EMP; -- EMP테이블의 SELECT절의 지정된 열만 출력
...
-- 4. 특정 행 출력 : WHERE절(조건절)에 논리연산자 : AND OR NOT
    ...
    SELECT * FROM EMP 
        WHERE TO_CHAR(HIREDATE,'RR/MM/DD') >='82/01/01' AND TO_CHAR(HIREDATE,'RR/MM/DD') <= '82/12/31';
...
SELECT EMPNO, ENAME, JOB, TO_CHAR(HIREDATE, 'RR/MM' ) FROM EMP WHERE HIREDATE >= '81/02';
SELECT EMPNO, ENAME, JOB, TO_DATE('81/02/01', 'RR/MM/DD' ) FROM EMP WHERE HIREDATE >= '81/02/01';
...
SELECT EMPNO, ENAME, JOB, SAL FROM EMP WHERE JOB='MANAGER' OR SAL>=2500;
--13.“ename은 XXX 업무이고 연봉은 XX다” 스타일로 모두 출력(연봉은 SAL*12+COMM)
SELECT ENAME || '은(는)' || JOB || '업무이고 연봉은' || (SAL*12+NVL(COMM,0)) || '이다' FROM EMP;
...
-- (2) 필드명 IN (값1, 값2, ..값N) 
    -- EX1. 부서번호가 10번이거나 30번이거나 40인 사람의 모든 정보
...
SELECT EMPNO,ENAME, JOB, MGR, HIREDATE, SAL, COMM, D.DEPTNO DNAME, LOC
    FROM EMP E, DEPT D
    WHERE E.DEPTNO=D.DEPTNO AND ENAME LIKE '%L%L%' AND (D.DEPTNO='30' OR MGR='7782');
