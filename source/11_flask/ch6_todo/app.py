...
# pip install flask pydantic cx_Oracle
# pip freeze > requirements.txt
# pip install -r requirements.txt

from flask import Flask, request, render_template
from database.repository import get_todos, get_next_id, get_todo
from database.repository import create_todo, update_todo
...
@app.route('/logout')
def logout():
    "로그아웃 성공 로직(세션 정보 제거)후 /todos로 이동"
...
def todos():
    ...
    next_id = get_next_id()
    return render_template('todo/stodos.html', todos=todos, next_id=next_id)
