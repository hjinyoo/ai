...
# 1. 변수 : {{var}} 또는 {{var | filter}} 사용
    # 기본 제공 필터 : lower, upper, length, capitalized, title, trim, replace
    # 형변환 제공 필터 : int, float, string
# 2. 제어문 {% %}
## 2-1 조건문 {% if 조건 %}A태그{% elif 조건2 %}B태그{% else %}C태그{% endif %}
## 2-2 반복문 
##    {% for var in vars %}
##      <태그>{{loop.index}}. {{var}}</태그>   
##        loop.index : 1부터 순번 / loop.first : 첫번쨰 loop인지 여부 / loop.last : 마지막 loop인지 여부
##    {% endfor %}
