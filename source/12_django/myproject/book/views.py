...
# 1. form없이 구현 2. form객체 생성 후 3. DjangoGenericView 이용 4. GenericView상속
...
def book_edit1(request, pk):
    # GET : 수젇 template페이지(book)만 응답
    # POST : 파라미터 받아 유효성 체크
        # 1) success : db에 update -> book:list
        # 2) fail : 오류메서지가 담긴 form 객체와 함께 수정 template 페이지로 이동
