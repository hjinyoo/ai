...
def custom_login(request):
    ...
    if request.method == "POST":
        ...
        if user is not None:
            ...
            request.session["username"] = username # 세션에 username 지정
            next_url = request.POST.get("next", "profile")
            return redirect(next_url)
        # else:
        #     form.add_error(None, "오류") # 로그인 실패시 오류 메세지 추가
    else:
        form = AuthenticationForm(
            request=request, 
            initial={"username":initial_username} # 회원가입 후 바로 로그인한 경우 username
            )
